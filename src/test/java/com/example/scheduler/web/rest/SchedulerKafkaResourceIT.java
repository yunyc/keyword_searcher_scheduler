package com.example.scheduler.web.rest;
...
import com.example.scheduler.IntegrationTest;
import com.example.scheduler.config.EmbeddedKafka;
import com.example.scheduler.config.KafkaSseConsumer;
import com.example.scheduler.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SchedulerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/scheduler-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/scheduler-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/scheduler-kafka/unregister"));
                return;
            }
        }
